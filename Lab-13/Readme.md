...
У цій лабораторній роботі ви попрацюєте з C-стилем рядків (`char*`), бібліотекою `<cstring>`, а також класом `std::string`. Для кожного завдання створіть окремий файл під `src/` (наприклад, `src/task01_string_length.cpp`, `src/task02_reverse_string.cpp`, …) з функцією `main`, що демонструє роботу.
